...
/*
 * A printer was chosen (or the dialog was cancelled). `user_data` is a strong
 * reference to the window, which keeps the PdfDemo it owns alive.
 */
static void
on_print_response(GtkDialog *dialog, int response, gpointer user_data)
{
    GtkWindow *window = user_data;
    PdfDemo   *demo = g_object_get_data(G_OBJECT(window), "pdf-demo");

    if (response != GTK_RESPONSE_OK || demo == NULL || demo->current_pdf == NULL) {
...
on_print_clicked(GtkButton *button, gpointer user_data)
{
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(on_print_response),
                     g_object_ref(demo->window));
