    ...
    pub(crate) fn new(rendering_context: Rc<dyn RenderingContext>, paint: &Paint) -> Self {
        ...
        let (mut webrender_renderer, webrender_api_sender) = webrender::create_webrender_instance(
            ...
            webrender::WebRenderOptions {
                ...
                // LOCAL PATCH (servo-gtk): must stay false on the swgl CPU
                // rasterizer. With it on, WebRender asks for the gradient
                // shader as "ps_quad_gradient DITHERING"
                // (webrender-0.70.0/src/renderer/shade.rs:866-875), but
                // swgl's build.rs only generates the bare `ps_quad_gradient`
                // variant, so `load_shader` returns null and swgl aborts in
                // BindAttribLocation (`assert(p.impl)`, gl.cc:1523) -- taking
                // the host process with it the moment a gradient is painted.
                // Costs a little banding in gradients; that is the trade.
                ...
                // LOCAL PATCH (servo-gtk): the CPU-only swgl rasterizer only
                // implements GL_LESS and GL_LEQUAL depth functions. WebRender's
                // quad-based picture-cache clear needs GL_ALWAYS (it draws a
                // quad at z=1.0 that must always pass and reset depth), so on
                // swgl it aborts in DepthFunc() -- and with asserts off the
                // quad is depth-rejected instead, leaving tiles never cleared.
                // Gecko does the same thing for Software WebRender. The
                // `other` arm uses a scissored glClear, which swgl supports.
